        ...
        if(!user){
          ...
          socket.on('createMsg', (msg, callback) =>{
             ...
             if (activeUser){
               ...
               io.to(activeUser.socketId).emit('newMsg', generateMsg(user._id, user.username, msg.msg));
        ...
        if(!doc){
          ...
          var lastMsgTime = new Date();
            var chatRoom = new ChatRoom({userIdOne: req.user._id, userIdTwo: userChat.userId,
              userNameOne: req.user.username, userNameTwo:req.body.userName, msgFromUserOne: emptyArray, msgFromUserTwo: emptyArray, lastMessageTime: lastMsgTime});
            ...
            Promise.all([query1,query2]).then(res.sendStatus(200)).catch((err) => console.log(err));
          ...
          UserChats.update(
            {userId: req.user._id},
            {$push: {chatIds: {_id: doc._id}}}).then((doc) =>
            {
              if(doc){
                res.sendStatus(200);
                return new Promise.resolve();
...
app.post('/user/chat/chatnames', [authenticateCookie, urlEncodedParser] , (req,res) =>{
      ...
      ChatRoom.find({'_id': { $in: myChatIds}}, {'_id': 0, 'userNameOne': 1, 'userNameTwo': 1, 'readByUserOne': 1, 'readByUserTwo': 1, 'lastMessageTime': 1}).sort({lastMessageTime: 'desc'}).then((chats) =>{
        res.json(chats);
      });
