  ...
  $.ajax({
       ...
       success: function(data , textStatus, request){
         ...
         $(data).each(function(i, chat){
          console.log('myName: '+myName+ ' 1: ' +chat.userNameOne + ' 2: ' + chat.userNameTwo);
           if(chat.userNameTwo === myName){
             if(!userExists(chat.userNameOne)){
               addUserIntoChats(i,chat.userNameOne, chat.readByUserTwo);
             ...
             if(chat.userNameOne === myName){
               if(!userExists(chat.userNameTwo)){
                addUserIntoChats(i,chat.userNameTwo, chat.readByUserOne);
...
//GET MSG FROM THE SERVER
  ...
  if(activeUser === msg.userName){
    ...
    if(!userExists(msg.userName)){
      ...
      addUserIntoChats(allUsers.length - 1,msg.userName, 1, true);
    } else {
...
$("#createMsg").on('click',function() {
  ...
  if (!msg.trim().length) {
    alert("Cannot send black message");
    return;
  }
...
function addUserIntoChats(displayId,from, num, prepend){
  ...
  $.ajax({
     ...
     //     $('#withName').css({ "opacity":"0.5"}).fadeTo("fast", 1);
       if(checkAndPushArray(userName, allUsers)){
        ...
        addUserIntoChats(allUsers.length-1,userName,0,true);
